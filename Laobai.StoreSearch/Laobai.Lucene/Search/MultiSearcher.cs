...
using ReaderUtil = Laobai.Lucene.Util.ReaderUtil;
...
namespace Laobai.Lucene.Search
{
	...
	public class MultiSearcher:Searcher
	{
		...
		{
			public AnonymousClassCollector(Laobai.Lucene.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				InitBlock(collector, start, enclosingInstance);
			}
			private void  InitBlock(Laobai.Lucene.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				this.collector = collector;
				this.start = start;
				this.enclosingInstance = enclosingInstance;
			}
			private Laobai.Lucene.Search.Collector collector;
			private int start;
		...
		public /*protected internal*/ override Weight CreateWeight(Query original)
		{
			...
			// step 2
		    ISet<Term> terms = Laobai.Lucene.Support.Compatibility.SetFactory.CreateHashSet<Term>();
