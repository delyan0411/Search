...
namespace Laobai.Lucene.Search.Spans
{
	...
	public class SpanNotQuery:SpanQuery, System.ICloneable
	{
		...
		{
			public AnonymousClassSpans(Laobai.Lucene.Index.IndexReader reader, SpanNotQuery enclosingInstance)
			{
				InitBlock(reader, enclosingInstance);
			}
			private void  InitBlock(Laobai.Lucene.Index.IndexReader reader, SpanNotQuery enclosingInstance)
			{
				this.reader = reader;
