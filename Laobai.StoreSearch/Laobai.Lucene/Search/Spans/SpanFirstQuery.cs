...
namespace Laobai.Lucene.Search.Spans
{
	...
	public class SpanFirstQuery : SpanQuery, System.ICloneable
	{
		...
		{
			public AnonymousClassSpans(Laobai.Lucene.Index.IndexReader reader, SpanFirstQuery enclosingInstance)
			{
				InitBlock(reader, enclosingInstance);
			}
			private void  InitBlock(Laobai.Lucene.Index.IndexReader reader, SpanFirstQuery enclosingInstance)
			{
				this.reader = reader;
