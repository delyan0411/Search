...
namespace Laobai.Lucene.Search.Spans
{
	...
	public class SpanOrQuery : SpanQuery, System.ICloneable
	{
		...
		{
			public AnonymousClassSpans(Laobai.Lucene.Index.IndexReader reader, SpanOrQuery enclosingInstance)
			{
				InitBlock(reader, enclosingInstance);
			}
			private void  InitBlock(Laobai.Lucene.Index.IndexReader reader, SpanOrQuery enclosingInstance)
			{
				this.reader = reader;
