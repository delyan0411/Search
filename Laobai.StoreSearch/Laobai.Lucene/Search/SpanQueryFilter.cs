...
using OpenBitSet = Laobai.Lucene.Util.OpenBitSet;
...
namespace Laobai.Lucene.Search
{
	...
	/// <p/>
	/// This filter does not cache.  See the <see cref="Laobai.Lucene.Search.CachingSpanFilter" /> for a wrapper that
	/// caches.
	/// 
	/// 
	/// </summary>
	...
	public class SpanQueryFilter:SpanFilter
	{
		...
		/// <c>query</c>.
		/// </summary>
		/// <param name="query">The <see cref="Laobai.Lucene.Search.Spans.SpanQuery" /> to use as the basis for the Filter.
		/// </param>
		...
		public override SpanFilterResult BitSpans(IndexReader reader)
		{
			...
			Laobai.Lucene.Search.Spans.Spans spans = internalQuery.GetSpans(reader);
