...
using ToStringUtils = Laobai.Lucene.Util.ToStringUtils;
using Explanation = Laobai.Lucene.Search.Explanation;
using Scorer = Laobai.Lucene.Search.Scorer;
...
using NearSpansOrdered = Laobai.Lucene.Search.Spans.NearSpansOrdered;
using NearSpansUnordered = Laobai.Lucene.Search.Spans.NearSpansUnordered;
using SpanNearQuery = Laobai.Lucene.Search.Spans.SpanNearQuery;
using SpanQuery = Laobai.Lucene.Search.Spans.SpanQuery;
using SpanScorer = Laobai.Lucene.Search.Spans.SpanScorer;
using SpanWeight = Laobai.Lucene.Search.Spans.SpanWeight;

namespace Laobai.Lucene.Search.Payloads
{
	
	/// <summary> This class is very similar to
	/// <see cref="Laobai.Lucene.Search.Spans.SpanNearQuery" /> except that it factors
	/// in the value of the payloads located at each of the positions where the
	/// <see cref="Laobai.Lucene.Search.Spans.TermSpans" /> occurs.
	/// <p/>
	/// In order to take advantage of this, you must override
	/// <see cref="Laobai.Lucene.Search.Similarity.ScorePayload" />
	/// which returns 1 by default.
	/// <p/>
	/// Payload scores are aggregated using a pluggable <see cref="PayloadFunction" />.
	/// 
	/// </summary>
	/// <seealso cref="Laobai.Lucene.Search.Similarity.ScorePayload">
	/// </seealso>
	[Serializable]
	public class PayloadNearQuery:SpanNearQuery, System.ICloneable
	{
		...
		public class PayloadNearSpanScorer:SpanScorer
		{
			...
			new internal Laobai.Lucene.Search.Spans.Spans spans;
			
			protected internal float payloadScore;
			private int payloadsSeen;
			internal Similarity similarity;
			
			protected internal PayloadNearSpanScorer(PayloadNearQuery enclosingInstance, Laobai.Lucene.Search.Spans.Spans spans, Weight weight, Similarity similarity, byte[] norms):base(spans, weight, similarity, norms)
			{
				InitBlock(enclosingInstance);
				this.spans = spans;
			}
			...
			public virtual void  GetPayloads(Laobai.Lucene.Search.Spans.Spans[] subSpans)
			{
			...
			public /*protected internal*/ override bool SetFreqCurrentDoc()
			{
				...
				Laobai.Lucene.Search.Spans.Spans[] spansArr = new Laobai.Lucene.Search.Spans.Spans[1];
				spansArr[0] = spans;
				payloadScore = 0;
				payloadsSeen = 0;
