...
namespace Laobai.Lucene.Search
{
	...
	public class SortField
	{
		...
		private Laobai.Lucene.Search.Parser parser;
		...
		/// </summary>
		public SortField(System.String field, Laobai.Lucene.Search.Parser parser):this(field, parser, false)
		{
		}
		...
		/// <summary>  subclass an existing numeric parser, or field is null
		/// </summary>
		public SortField(System.String field, Laobai.Lucene.Search.Parser parser, bool reverse)
		{
			if (parser is Laobai.Lucene.Search.IntParser)
				InitFieldType(field, INT);
			else if (parser is Laobai.Lucene.Search.FloatParser)
				InitFieldType(field, FLOAT);
			else if (parser is Laobai.Lucene.Search.ShortParser)
				InitFieldType(field, SHORT);
			else if (parser is Laobai.Lucene.Search.ByteParser)
				InitFieldType(field, BYTE);
			else if (parser is Laobai.Lucene.Search.LongParser)
				InitFieldType(field, LONG);
			else if (parser is Laobai.Lucene.Search.DoubleParser)
