...
using DocIdSet = Laobai.Lucene.Search.DocIdSet;
using DocIdSetIterator = Laobai.Lucene.Search.DocIdSetIterator;

namespace Laobai.Lucene.Util
{
	...
	/// borrowed from <see cref="Laobai.Lucene.Store.IndexInput" /> and
	/// <see cref="Laobai.Lucene.Store.IndexOutput" />.<p/>
	...
	public class SortedVIntList:DocIdSet
	{
		private class AnonymousClassDocIdSetIterator:DocIdSetIterator
		{
			...
			{
				// See Laobai.Lucene.Store.IndexInput.readVInt()
				sbyte b = Enclosing_Instance.bytes[bytePos++];
				lastInt += (b & Laobai.Lucene.Util.SortedVIntList.VB1);
				for (int s = Laobai.Lucene.Util.SortedVIntList.BIT_SHIFT; (b & ~ Laobai.Lucene.Util.SortedVIntList.VB1) != 0; s += Laobai.Lucene.Util.SortedVIntList.BIT_SHIFT)
				{
					b = Enclosing_Instance.bytes[bytePos++];
					lastInt += ((b & Laobai.Lucene.Util.SortedVIntList.VB1) << s);
				}
			}
		...
		private class SortedVIntListBuilder
		{
			...
			internal virtual void  AddInt(int nextInt)
			{
				...
				// See Laobai.Lucene.Store.IndexOutput.writeVInt()
				while ((diff & ~ Laobai.Lucene.Util.SortedVIntList.VB1) != 0)
				{
					// The high bit of the next byte needs to be set.
					Enclosing_Instance.bytes[Enclosing_Instance.lastBytePos++] = (sbyte) ((diff & Laobai.Lucene.Util.SortedVIntList.VB1) | ~ Laobai.Lucene.Util.SortedVIntList.VB1);
					diff = Number.URShift(diff, Laobai.Lucene.Util.SortedVIntList.BIT_SHIFT);
				}
