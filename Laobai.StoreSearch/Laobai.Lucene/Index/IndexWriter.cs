...
using Analyzer = Laobai.Lucene.Analysis.Analyzer;
using Document = Laobai.Lucene.Documents.Document;
using IndexingChain = Laobai.Lucene.Index.DocumentsWriter.IndexingChain;
using AlreadyClosedException = Laobai.Lucene.Store.AlreadyClosedException;
using BufferedIndexInput = Laobai.Lucene.Store.BufferedIndexInput;
using Directory = Laobai.Lucene.Store.Directory;
using Lock = Laobai.Lucene.Store.Lock;
using LockObtainFailedException = Laobai.Lucene.Store.LockObtainFailedException;
using Constants = Laobai.Lucene.Util.Constants;
...
namespace Laobai.Lucene.Index
{
    ...
    */
    public class IndexWriter : System.IDisposable
	{
        ...
        private ISet<SegmentInfo> segmentsToOptimize = Laobai.Lucene.Support.Compatibility.SetFactory.CreateHashSet<SegmentInfo>(); // used by optimize to note those needing optimization
		...
		private ISet<MergePolicy.OneMerge> runningMerges = Laobai.Lucene.Support.Compatibility.SetFactory.CreateHashSet<MergePolicy.OneMerge>();
		...
		/// using <see cref="IndexReader.Open(Laobai.Lucene.Store.Directory, bool)" /> to open a new reader. But the turarnound
		...
		/// <param name="deletionPolicy">see <a href="#deletionPolicy">above</a>
		/// </param>
		/// <param name="mfl"><see cref="Laobai.Lucene.Index.IndexWriter.MaxFieldLength" />, whether or not to limit field lengths.  Value is in number of terms/tokens
		/// </param>
		/// <throws>  CorruptIndexException if the index is corrupt </throws>
		/// <throws>  LockObtainFailedException if another writer </throws>
		/// <summary>  has this index open (<c>write.lock</c> could not
		...
		/// <param name="deletionPolicy">see <a href="#deletionPolicy">above</a>
		/// </param>
		/// <param name="mfl">whether or not to limit field lengths, value is in number of terms/tokens.  See <see cref="Laobai.Lucene.Index.IndexWriter.MaxFieldLength" />.
		/// </param>
		...
		public virtual void  Optimize(int maxNumSegments, bool doWait)
		{
			...
			lock (this)
			{
                ...
                segmentsToOptimize = Laobai.Lucene.Support.Compatibility.SetFactory.CreateHashSet<SegmentInfo>();
		...
		* addIndexesNoOptimize(). */
		...
		public sealed class MaxFieldLength
		{
			...
			{
				LIMITED = new MaxFieldLength("LIMITED", Laobai.Lucene.Index.IndexWriter.DEFAULT_MAX_FIELD_LENGTH);
			}
		}
