...
using ChecksumIndexInput = Laobai.Lucene.Store.ChecksumIndexInput;
using ChecksumIndexOutput = Laobai.Lucene.Store.ChecksumIndexOutput;
using Directory = Laobai.Lucene.Store.Directory;
using IndexInput = Laobai.Lucene.Store.IndexInput;
using IndexOutput = Laobai.Lucene.Store.IndexOutput;
using NoSuchDirectoryException = Laobai.Lucene.Store.NoSuchDirectoryException;

namespace Laobai.Lucene.Index
{
	...
	public sealed class SegmentInfos : List<SegmentInfo>, ICloneable
	{
		private class AnonymousClassFindSegmentsFile:FindSegmentsFile
		{
			...
			internal AnonymousClassFindSegmentsFile(SegmentInfos enclosingInstance, Laobai.Lucene.Store.Directory Param1):base(Param1)
			{
			...
			public System.Object Run(IndexCommit commit)
			{
				...
				while (true)
				{
					...
					if (0 == method)
					{
						...
						if (files != null)
							genA = Laobai.Lucene.Index.SegmentInfos.GetCurrentSegmentGeneration(files);
						
						Laobai.Lucene.Index.SegmentInfos.Message("directory listing genA=" + genA);
						...
						long genB = - 1;
						for (int i = 0; i < Laobai.Lucene.Index.SegmentInfos.defaultGenFileRetryCount; i++)
						{
							...
							{
								Laobai.Lucene.Index.SegmentInfos.Message("segments.gen open: FileNotFoundException " + e);
								break;
							}
							catch (System.IO.IOException e)
							{
								Laobai.Lucene.Index.SegmentInfos.Message("segments.gen open: IOException " + e);
							}
							...
							if (genInput != null)
							{
								try
								{
									...
									if (version == Laobai.Lucene.Index.SegmentInfos.FORMAT_LOCKLESS)
									{
										long gen0 = genInput.ReadLong();
										long gen1 = genInput.ReadLong();
										Laobai.Lucene.Index.SegmentInfos.Message("fallback check: " + gen0 + "; " + gen1);
										if (gen0 == gen1)
										{
                            ...
                            System.Threading.Thread.Sleep(new TimeSpan((System.Int64) 10000 * Laobai.Lucene.Index.SegmentInfos.defaultGenFileRetryPauseMsec));
							
							
						}
						
						Laobai.Lucene.Index.SegmentInfos.Message(IndexFileNames.SEGMENTS_GEN + " check: genB=" + genB);
					...
					if (1 == method || (0 == method && lastGen == gen && retry))
					{
						...
						if (genLookaheadCount < Laobai.Lucene.Index.SegmentInfos.defaultGenLookaheadCount)
						{
							gen++;
							genLookaheadCount++;
							Laobai.Lucene.Index.SegmentInfos.Message("look ahead increment gen to " + gen);
						}
					}
					...
					try
					{
						...
						Laobai.Lucene.Index.SegmentInfos.Message("success on " + segmentFileName);
					...
					catch (System.IO.IOException err)
					{
						...
						Laobai.Lucene.Index.SegmentInfos.Message("primary Exception on '" + segmentFileName + "': " + err + "'; will retry: retry=" + retry + "; gen = " + gen);
						...
						if (!retry && gen > 1)
						{
							...
							if (prevExists)
							{
								Laobai.Lucene.Index.SegmentInfos.Message("fallback to prior segment file '" + prevSegmentFileName + "'");
								try
								{
									System.Object v = DoBody(prevSegmentFileName);
									if (exc != null)
									{
										Laobai.Lucene.Index.SegmentInfos.Message("success on fallback " + prevSegmentFileName);
									}
									return v;
								}
								catch (System.IO.IOException err2)
								{
									Laobai.Lucene.Index.SegmentInfos.Message("secondary Exception on '" + prevSegmentFileName + "': " + err2 + "'; will retry");
								}
							}
						}
					}
        ...
        #region Laobai.Lucene (Equals & GetHashCode )
        /// <summary>
