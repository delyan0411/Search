...
{
    public class WmProductWriter
    {
        
        private IndexUtils indexUtils;
        public WmProductWriter()
        {
            this.indexUtils = new IndexUtils("wmproduct");//初始化
        }
        ...
        private List<Field> FormatFields(string sname, string scommon, string sbrand
            , string skey, string strait, string stype, string stext, string slicense, string sstore,string sdepart,string ssymp)
        {
            ...
            list.Add(this.indexUtils.AnalyzedField("sdepart", sdepart, 1.00f));//科室
            list.Add(this.indexUtils.AnalyzedField("ssymp", ssymp, 1.00f));//病症
        ...
        private Document CreateDocument(ProductWmInfo item)
        {
            ...
            var list = this.FormatFields(item.product_name, item.common_name, item.product_brand
                ...
                , item.product_func, item.product_license, item.shop_name,item.departments,item.symptom);
        ...
        /// <returns></returns>
        private bool IsFilter(ProductWmInfo item)
        {
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private bool IsBadDocument(ProductWmInfo item)
        {
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private bool MatchRule(ProductWmInfo item)
        {
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private float GetTotalScore(ProductWmInfo item)
        {
            //float score = 0f;
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private float GetGoodRate(ProductWmInfo item)
        {
            if (this.IsBadDocument(item))
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private float GetSalesIndex(ProductWmInfo item)
        {
            if (this.IsBadDocument(item))
                return badScore;//如果是下架或无库存的商品评分为-10
            float score = 0f;
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private float GetCommentIndex(ProductWmInfo item)
        {
            if (this.IsBadDocument(item))
                return badScore;//如果是下架或无库存的商品评分为-10
            //float score = 0f;
        ...
        /// <returns></returns>
        private List<ProductWmInfo> ResetList(List<ProductWmInfo> list)
        {
            List<ProductWmInfo> newList = new List<ProductWmInfo>();
            List<string> idList = new List<string>();//需要删除的索引内容
            foreach (ProductWmInfo p in list)
            {
        ...
        /// </summary>
        /// <param name="list"></param>
        public void CreateIndex(List<ProductWmInfo> list)
        {
            list = this.ResetList(list);
            if (list.Count < 1)
                return;
            List<SetLuceneDocument> docs = new List<SetLuceneDocument>();
            foreach (ProductWmInfo item in list)
            {
